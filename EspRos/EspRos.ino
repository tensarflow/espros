...
{ // Better sgn function definition
  if (val < 0)
    return -1;
  if (val == 0)
    return 0;
...
void tic(void *pArg)
{ // timerCallback, repeat every "period"
  lv = (lmc - lmc0);
  lmc0 = lmc;
  rv = (rmc - rmc0);
  rmc0 = rmc;
  ...
  rIn = rv * kt;
  if (abs(lv) >= 1)
    lIn = abs(lvt) * sgn(lv); // use timing to calculate velocity only if ticks are greater than one
  if (abs(rv) >= 1)
    rIn = abs(rvt) * sgn(rv); // to avoid problems with sign and inertia for a simple encoder (no quadrature)
  lPID.Compute();
  rPID.Compute();
  motion(lOut, rOut);
}
...
{                                       // servo 80 = 0 deg, servo -80 = - 90deg, servo 160 = 90 deg
